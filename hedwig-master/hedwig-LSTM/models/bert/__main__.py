...
class BertForClassificationWithLSTM(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels=2):
        ...
        self.hidden_dim = config.hidden_size # config.hidden_size # int(config.hidden_size/2)
        self.hidden_dim2 = 24
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.lstm = nn.LSTM(config.hidden_size, self.hidden_dim, num_layers=1, bidirectional=True) # 수정 요망
        self.linear1 = nn.Linear(self.hidden_dim*2, self.hidden_dim2)
        self.linear2 = nn.Linear(self.hidden_dim2 + 6, config.num_labels)
...
#         self.classifier = nn.Linear(self.hidden_dim*2, config.num_labels)
    ...
    def normalize(self, x):
        mean = torch.tensor([7.82, 166.06, 3.58, 0.20, 14.85, 5.77]).unsqueeze(0).cuda()
...
#     @add_start_docstrings_to_callable(BERT_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, meta_data=None,
        ...
        pooled_output = torch.stack(outputs, dim=0) # seq_len * batch_size * 768
        ...
        if meta_data is not None:
            logits = torch.cat([logits, self.normalize(meta_data)], dim=1)
