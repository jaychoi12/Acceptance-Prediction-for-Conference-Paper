...
class BertEvaluator(object):
    ...
    def get_scores(self, silent=False):
        ...
        if eval_features[0].meta_data is not None:
            meta_data = torch.tensor([f.meta_data for f in eval_features], dtype=torch.float)
            eval_data = TensorDataset(padded_input_ids, padded_input_mask, padded_segment_ids, label_ids, meta_data)
        else:
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", disable=silent):
            ...
            if len(batch) == 5:
                ...
                meta_data = meta_data.to(self.args.device)
            else:
