...
class BertTrainer(object):
    ...
    def train_epoch(self, train_dataloader):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            if len(batch) == 5:
                input_ids, input_mask, segment_ids, label_ids, meta_data = batch
            else:
            ...
            if isinstance(self.model, BertForSequenceClassification):
                ...
                logits = self.model(input_ids, input_mask, segment_ids, meta_data)
...
#         unpadded_meta_data = [f.meta_data for f in train_features]
        ...
        if train_features[0].meta_data is not None:
            meta_data = torch.tensor([f.meta_data for f in train_features], dtype=torch.float)
            train_data = TensorDataset(padded_input_ids, padded_input_mask, padded_segment_ids, label_ids, meta_data)
        else:
        ...
        for epoch in trange(int(self.args.epochs), desc="Epoch"):
            ...
            train_evaluator = BertEvaluator(self.model, self.processor, self.tokenizer, self.args, split='train')
            train_acc, train_precision, train_recall, train_f1, train_loss = train_evaluator.get_scores()[0]
