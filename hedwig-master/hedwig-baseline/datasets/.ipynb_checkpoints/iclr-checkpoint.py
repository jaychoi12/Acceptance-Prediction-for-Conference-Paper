...
def char_quantize(string, max_length=500):
    identity = np.identity(len(ICLRCharQuantized.ALPHABET))
    quantized_string = np.array([identity[ICLRCharQuantized.ALPHABET[char]] for char in list(string.lower()) if char in ICLRCharQuantized.ALPHABET], dtype=np.float32)
    if len(quantized_string) > max_length:
        return quantized_string[:max_length]
    else:
        return np.concatenate((quantized_string, np.zeros((max_length - len(quantized_string), len(ICLRCharQuantized.ALPHABET)), dtype=np.float32)))


class ICLR(TabularDataset):
    NAME = 'ICLR'
    NUM_CLASSES = 2
    IS_MULTILABEL = False
    ...
    @classmethod
    def splits(cls, path, train=os.path.join('ICLR', 'train.tsv'),
               validation=os.path.join('ICLR', 'dev.tsv'),
               test=os.path.join('ICLR', 'test.tsv'), **kwargs):
        return super(ICLR, cls).splits(
...
class ICLRHierarchical(ICLR):
    NESTING_FIELD = Field(batch_first=True, tokenize=clean_string)
    TEXT_FIELD = NestedField(NESTING_FIELD, tokenize=split_sents)


class ICLRCharQuantized(ICLR):
